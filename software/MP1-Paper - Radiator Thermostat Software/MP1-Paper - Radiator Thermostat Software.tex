...
Classical MVP relies on rich, automated UI widgets (Interactors) that generate high-level semantic events---an assumption common to modern application frameworks, including desktop environments (Windows, Taligent's CommonPoint \cite{Kratochvil.2011}), mobile platforms (Android, iOS), and web frameworks. In the bare-metal embedded context of this work, no such widget infrastructure is available. User input must therefore be handled manually, closer to the approach taken in MVC.
