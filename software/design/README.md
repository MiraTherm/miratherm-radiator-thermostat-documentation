...
To ensure correct order of the program flow, tasks communicate using event queues for signaling and synchronizing state changes (e.g., using requests). Each event queue is intended for one-directional communication between two tasks. For bidirectional communication, two event queues are used.
