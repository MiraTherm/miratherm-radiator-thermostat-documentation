...
\label{subsec:REQ 1.1: Prototype Display}
...
\label{subsec:REQ 2.1: Prototype Rotary Encoder}
...
\label{subsec:REQ 3: Push Buttons}
...
\label{subsec:REQ 3.1: Prototype Push Buttons}
...
\label{subsec:REQ 4.1: Prototype Motor Control}
...
\label{subsec:REQ 5.1: Prototype Motor Current Measurement}
...
\label{subsec:REQ 6.1: Prototype Power Supply Voltage Measurement}
...
\label{subsec:REQ 7: Motor Current Extremes Definition}
...
\label{subsec:REQ 8: Temperature Measurement}
...
\label{subsec:REQ 8.1: Prototype Temperature Sensor}
...
\label{subsec:REQ 9: Configuration Routines}
The system shall provide the following configuration routines for initial setup:
\begin{itemize}
    \item \textbf{\ac{cva}}: Partially automatic, wireless configuration via Matter standard and/or Thread protocol.
...
\subsection*{REQ 9.1: Configuration Routine Choice}
...
\subsection*{REQ 9.2: Prototype Configuration}
\label{subsec:REQ 9.2: Prototype Configuration}
In the prototype version of the thermostat software, only the \acs{cod} shall be implemented. Wireless connectivity and the \acs{cva} shall not be supported.

\subsection*{REQ 9.3: Configuration Via App Blocking}
\label{subsec:REQ 9.3: Configuration Via App Blocking}
...
\label{subsec:REQ 10: Date and Time Configuration}
...
\label{subsec:REQ 11: Installation Command}
...
\label{subsec:REQ 12: Valve Adaptation}
...
\label{subsec:REQ 13: Daily Schedule Configuration}
...
\label{subsec:REQ 14: Main Display Page}
...
\label{subsec:REQ 15: Configuration Menu}
...
\label{subsec:REQ 16: Inactivity Timeout}
If no user interactions occur for 30 seconds, the system shall turn the display off. The system shall return to the currently active page upon the next user interaction via any button.

\subsection*{REQ 17: Operational Modes}
\label{subsec:REQ 17: Operational Modes}
...
\label{subsec:REQ 18: Switching Operational Modes}
The system shall allow switching between Manual Mode and Auto Mode using the mode button.

\subsection*{REQ 19: Boost Mode}
\label{subsec:REQ 19: Boost Mode}
The system shall provide a boost mode that immediately opens the heating valve to 80\% for 5 minutes after double-pressing the control wheel button. Then the system shall display the remaining time instead of the target temperature on the main display page. The remaining time shall be displayed as a countdown in seconds. The function shall be deactivatable at any time by pressing the control wheel button.

\subsection*{REQ 20: Temperature Range and Resolution}
\label{subsec:REQ 20: Temperature Range and Resolution}
...
\label{subsec:REQ 21: Temperature Offset Configuration}
...
\label{subsec:REQ 22: Automatic Summer/Winter Time Switching}
...
\label{subsec:REQ 25: Automatic Descaling Routine}
...
% \label{subsec:REQ 26: Motor Direction Indication}
...
% \label{subsec:REQ 27: Weekly Program for Auto Mode}
...
% \label{subsec:REQ 28: Manual Temperature Control in Auto Mode}
...
% \label{subsec:REQ 29: Open-Window Detection}
...
% \label{subsec:REQ 30: Matter-Standard Connectivity Control}
...
% \label{subsec:REQ 31: Holiday Function}
...
% \label{subsec:REQ 32: Comfort and Eco Temperature Settings}
...
% \label{subsec:REQ 33: Heating Pause (Battery Saving Mode)}
...
% \label{subsec:REQ 34: Frost Protection Mode}
...
% \label{subsec:REQ 35: Child Safety Lock}
...
% \label{subsec:REQ 36: Application-Based Control}
...
% \label{subsec:REQ 37: Bidirectional Wireless Communication and Synchronization}
...
% \label{subsec:REQ 38: Adaptive Control}
...
% \label{subsec:REQ 39: Wireless Signal Strength Indication}
...
\subsection*{REQ 26: Energy Efficiency}
\label{subsec:REQ 26: Energy Efficiency}
...
\subsection*{REQ 27: User Interface Responsiveness}
\label{subsec:REQ 27: User Interface Responsiveness}
The system shall provide immediate visual feedback for all user interactions via the control wheel and buttons, with display updates occurring within acceptable latency for user perception.

% \subsection*{REQ 28: Battery Life}
% \label{subsec:REQ 28: Battery Life}
