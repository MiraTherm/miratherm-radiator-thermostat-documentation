...
The system shall integrate a rotary encoder KY-040 via \acp{gpio}.
...
The system shall measure the power supply voltage using an \ac{adc} channel. The measured voltage shall be used to calculate the battery charge level as a percentage.
...
\subsection*{REQ 7.1: Pin Position Extremes Definition}
\label{subsec:REQ 7.1: Pin Position Extremes Definition}
...
\subsection*{REQ 8.1: Prototype Configuration}
\label{subsec:REQ 8.1: Prototype Configuration}
...
\subsection*{REQ 8.2: Configuration Routine Choice}
\label{subsec:REQ 8.2: Configuration Routine Choice}
...
\begin{itemize}
    \item \textbf{OFF state}: When the user sets a target temperature below $5.0^\circ C$, the valve shall be fully closed.
    \item \textbf{ON state}: When the user sets a target temperature above $30.0^\circ C$, the valve shall be fully opened.
\end{itemize}
...
\subsection*{REQ 25: Power Supply and Battery Life}
\label{subsec:REQ 25: Power Supply and Battery Life}
...
The system shall provide immediate visual feedback for all user interactions via the control wheel, SW1, and SW2 buttons, with display updates occurring within acceptable latency for user perception.